...
namespace meta_scene::importers::pbrt {
	...
	render_config::render_config() 
	{
		...
		material->properties["diffuse"].constant.value = spectrum(0.5);
	...
	std::optional<meta_scene::objects::material> scene_state::find_material(const std::string& name)
	{
		...
		warn("the material (name = {0}) is not found. we will return nullptr.", name);
	...
	std::optional<meta_scene::objects::medium> scene_state::find_medium(const std::string& name)
	{
		if (name == "") return std::nullopt;
	...
	meta_scene::objects::texture scene_state::find_texture(const std::string& name)
	{
		if (textures.find(name) != textures.end()) return textures.at(name);

		warn("the texture (name = {0}) is not found. we will return constant texture.", name);

		meta_scene::objects::texture texture;

		texture.type = "constant";
		texture.constant.value = vector3(0);
	...
	objects scene_state::find_object(const std::string& name)
	{
